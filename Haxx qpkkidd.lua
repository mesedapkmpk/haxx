...
note.Position = UDim2.new(0.088, 0,0.83, 0)
...
logo.Position = UDim2.new(-0.257993365, 0, 0.8, 0)
...
ImageButton.Position = UDim2.new(0.184615389, 0, 0, 0)

ImageButton.Size = UDim2.new(0, 25, 0, 25)

ImageButton.Image = "http://www.roblox.com/asset/?id=8388262491"
...
AddCommand("ClearStarter(mayday)", function()
...
AddCommand("furry", function()

loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Undetectable-furrygui-v3-by-N0thing-Repost-31615"))()
...
AddCommand("Baseplate", function() 
	...
	for _, v in pairs(workspace:GetChildren()) do
		...
		if player then
			-- Oyuncu karakteriyse atla -- Eğer `continue` desteklenmiyorsa, alttaki gibi `else` kullanacağız
		    else
			for _, instance in pairs({v, table.unpack(v:GetDescendants())}) do
				if instance:IsA("BasePart") then
					local isRaycastHit = rc and rc.Instance == instance
					local isTooLarge = instance.Size.Magnitude >= 1000
					local isBaseplateNamed = instance.Name:lower():gsub(" ", ""):find("baseplate")

					if not (isRaycastHit or isTooLarge or isBaseplateNamed) then
						work(instance)
					end
				end
			end
...
mainFrame.Position = UDim2.new(0.5, 0, 0.5, 0) -- Başlangıçta ortada
mainFrame.Visible = true

-- Çok aşağı kaydırma (örneğin ekranın 2 katı kadar aşağı)
local tweenOut = TweenService:Create(mainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    Position = UDim2.new(0.5, 0, 2, 0) -- 2 = ekranın 2 katı aşağı
})

local tweenIn = TweenService:Create(mainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Position = UDim2.new(0.5, 0, 0.5, 0) -- Ekran ortası
})
